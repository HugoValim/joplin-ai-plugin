...
const archiveExternals = {};
for (const moduleName of builtinModules) {
  moduleFallback[moduleName] = false;
  const plainName = moduleName.replace(/^node:/, "");
  archiveExternals[moduleName] = `commonjs ${plainName}`;
  archiveExternals[`node:${plainName}`] = `commonjs ${plainName}`;
}
...
function validateScreenshots(screenshots) {
  ...
  for (const screenshot of screenshots) {
    ...
    if (
      screenshot.src.startsWith("https://") ||
      screenshot.src.startsWith("http://")
    ) {
      continue;
    }
