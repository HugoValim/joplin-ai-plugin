...
const NOTE_WRITING_POLICY = [
  ...
  "When the user asks to create, move, rename, reorganize, delete, or restore notes or notebooks, or says apply, do it, go ahead, or proceed: call the matching propose-write tools in this turn after the minimum reads needed for IDs and updated_time.",
  "For multi-notebook or multi-note requests: inventory with list/search tools, call set_agent_plan with a checklist of small items (at most 5 notes each), then execute pending items in bounded batches (read at most 5 bodies, propose edits, stop for ChangeReview). Do not ask the user to shrink scope when the plan can drive batching across apply/continue cycles.",
  ...
  "Do not narrate upcoming tool calls in chat. Call propose-write and plan-update tools in the same turn you decide to use them. Do not stop at a chat-only text plan or ask whether to apply in chat; use set_agent_plan / update_agent_plan_item for progress, and ChangeReview for write review.",
  ...
  "Deletion always requires explicit user review and moves items to Joplin Trash. Never request permanent deletion. Use list_trash plus restore_note or restore_notebook to recover soft-deleted items through the same review gates.",
