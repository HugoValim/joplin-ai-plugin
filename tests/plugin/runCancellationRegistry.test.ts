...
describe("RunCancellationRegistry.isBusy", () => {
  test("reports true when any chat has an active run", () => {
    const registry = new RunCancellationRegistry();
    const controller = new AbortController();

    expect(registry.isBusy()).toBe(false);
    registry.replace("chat-other", "run-1", controller);
    expect(registry.isBusy()).toBe(true);

    registry.clearIfCurrent("chat-other", controller);
    expect(registry.isBusy()).toBe(false);
  });
});
